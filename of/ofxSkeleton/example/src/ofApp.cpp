...
void ofApp::update(){
  ...
  ofxSkeleton::CHUNK_SIZE  = (int)ui_chunkSize;
  ofxSkeleton::SAVE_RECTS  = (int)ui_drawRects;
  ...
  if (ui_useCam){
    ...
    if (cap.isFrameNew()){
      ...
      // Trace the Skeleton!
      // ========================
      polylines = ofxSkeleton::trace(preprocessed.getPixels());
      // ofxSkeleton::trace returns a vector<vector<ofVec2f>>
    }
    ...
    if (vid.isFrameNew()){
      ...
      // Trace the Skeleton!
      // ========================
      polylines = ofxSkeleton::trace(im);
    }
  }
...
void ofApp::draw(){
  ...
  if (ui_drawRects){
    // ofxSkeleton::getRects() gives the rects from previous run
    ...
    // also make sure ofxSkeleton::SAVE_RECTS == 1 otherwise this will be empty
    ofxSkeleton::draw(polylines, ofxSkeleton::getRects());
  }else{
    // ofxSkeleton::draw can also be called without the rects
    ofxSkeleton::draw(polylines);
  }
